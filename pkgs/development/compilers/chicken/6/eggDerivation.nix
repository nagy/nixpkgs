...
(stdenv.mkDerivation (
  {
    ...
    env = {
      ...
      # CHICKEN 6's chicken-install creates and locks a cache directory
      # during installation (see NEWS for 6.0.0).  The sandbox HOME is not
      # writable, so point the cache at TMPDIR.
      CHICKEN_EGG_CACHE = "$TMPDIR";
    }
    // (args.env or { });
