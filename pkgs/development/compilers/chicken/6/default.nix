...
let
  # Some eggs mistakenly declare dependencies on modules which are part of
  # chicken itself and thus need not (and cannot) be installed as eggs.
  # Instead of marking such eggs as broken, we remove these invalid
  # dependencies.
  invalidDependencies = [ ];
in
lib.makeScope newScope (self: {
    ...
    lib.makeScope self.newScope (
      ...
      (lib.mapAttrs (
        ...
        self.eggDerivation {
          ...
          meta.homepage = "https://wiki.call-cc.org/eggref/6/${pname}";
  ...
  # egg2nix is not packaged for CHICKEN 6: its version resolution relies on
  # `chicken-install -list-versions`, which is broken in the 6.0.0 release
  # (the henrietta HTTP response is fetched but parsed as empty).  See
  # ../5/egg2nix.nix for the CHICKEN 5 version.
